...
class ViewGUI():
    ...
    def __init__(self, window_root, default_path):
        ...
        #　メインウィンドウタイトル
        self.window_root.title('GUI Image Editor v1.0.0')
        ...
        self.window_sub_canvas = tk.Canvas(self.tab1, height=450, width=400, bg='gray')
        # Video[tab2]
        ...
        # Init
        canvas_dict = {'Photo':self.window_sub_canvas, 'Video1':self.window_video_canvas1, 'Video2':self.window_video_canvas2}
    ...
    def event_selectfile(self, event):
        if self.control.GetEventState('set'):
            print(sys._getframe().f_code.co_name)
    ...
    def event_rotate(self, idx):
        ...
        def check_event():
            if self.control.GetEventState('edit'):
    ...
    def event_clip_try(self):
        if self.control.GetEventState('clip'):
            print(sys._getframe().f_code.co_name)
        
        
    def event_clip_done(self):
        if self.control.GetEventState('done'):
            print(sys._getframe().f_code.co_name)
            self.control.Edit(self.select_tab, 'clip_done')
    
    
    def event_clip_start(self, event):
        if self.control.GetEventState('rect'):
    ...
    def event_save(self):
        if self.control.GetEventState('save|undo'):
            print(sys._getframe().f_code.co_name)
    ...
    # Photo        
    def event_prev(self):
        if self.control.GetEventState('prev'):
            print(sys._getframe().f_code.co_name)
            pos = self.control.DrawPhoto('prev')
            self.combo_file.set(self.file_list[pos])
        
        
    def event_next(self):
        if self.control.GetEventState('next'):
            print(sys._getframe().f_code.co_name)
            pos = self.control.DrawPhoto('next')
            self.combo_file.set(self.file_list[pos])
    ...
    def update_timestamp(self, status, frame_no, h,m,s):
        ...
        if status:
            ...
            self.control.SetEventState(2)
            
    
    def update_frameno(self, status, frame_no, h,m,s):
        canvas_id = int(self.select_canvas.replace('Video',''))
        idx = canvas_id -1
        if status:
